...
class BenchmarkResults:
    ...
    def add_error(self, filename, error_msg, error_type="EXTRACTION_FAILED"):
        self.errors.append({"filename": filename, "error": error_msg, "type": error_type})
    ...
    def add_result(self, filename, ground_truth, predicted, features_extracted):
        ...
        self.log_results.append({
            ...
            "predicted": predicted,
            "predicted_confidence": [p.get("confidence") for p in predicted] if isinstance(predicted, list) else []
        })
        
    def compute_metrics(self) -> dict:
        self.overall["total_logs"] = self.overall["successful_extractions"] + self.overall["failed_extractions"]
        ...
        for res in self.log_results:
            gt = set(res["ground_truth"])
            pred = set([d["failure_type"] for d in res["predicted"]])
            if gt == pred:
                exact_matches += 1
                
        self.overall["accuracy"] = float(exact_matches / max(self.overall["successful_extractions"], 1))
    ...
    def to_markdown(self) -> str:
        ...
        for label, vals in metrics["per_label"].items():
            if vals["support"] > 0 or vals["fp"] > 0:
                lines.append(f"| {label} | {vals['support']} | {vals['tp']} | {vals['precision']:.2f} | {vals['f1']:.2f} |")
        ...
        for conf in metrics["confusion_details"][:10]:
            lines.append(f"- **{conf['filename']}**: Expected {conf['ground_truth']} vs. Predicted {conf['predicted']}")
