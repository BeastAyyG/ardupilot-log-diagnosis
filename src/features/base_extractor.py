...
class BaseExtractor(ABC):
    ...
    def _safe_stats(self, values: list, times: list = None, threshold: float = None, mode: str = "above") -> dict:
        """Compute mean/max/min/std/range safely and tanomaly if times provided.
        Returns zeros if values is empty."""
        if not values or len(values) == 0:
            return {"mean": 0.0, "max": 0.0, "min": 0.0, "std": 0.0, "range": 0.0, "tmax": 0.0, "tanomaly": -1.0}
        ...
        if times and len(times) == len(values):
            ...
            if mode == "above":
                test_threshold = threshold if threshold is not None else (res["mean"] + 2 * res["std"])
