...
Tooling in the ArduPilot ecosystem mostly targets prevention rather than
post-crash diagnosis. The ArduPilot Methodic Configurator \citep{amc}, for
example, walks users through a fixed configuration and tuning sequence so that
many setup errors never reach flight. It does not classify the root cause of a
crash from a flight log, which is the task benchmarked here.
