...
[![Real-log macro F1 (incident-grouped, v3): 0.06-0.07, chance 0.08](https://img.shields.io/badge/real--log%20macro%20F1%20(incident--grouped)-0.06--0.07%20%7C%20chance%200.08-red)](docs/EVIDENCE_LEDGER.md)
...
CITA is this project's approach to a known problem in automated crash analysis: **post-crash noise**.

> **Measured status:** CITA shows **no measurable benefit**. On benchmark v1 it changed 20 of 41 top predictions, and 8 were correct either way. On the thread-verified labels, rules with CITA get 0 of 22 right on v2 (1 without) and 0 of 34 on the pre-registered v3 (3 without) ([evidence](docs/EVIDENCE_LEDGER.md)). The labels are provisional and have no onset annotations. A proper test needs annotated onset times, which is planned in [`docs/PUBLICATION_PLAN.md`](docs/PUBLICATION_PLAN.md). Treat the description below as the design intent.
...
**Intended result:** a vibration spike at T-45s that cascades into EKF divergence at T-20s should be labelled `vibration_high`, not `ekf_failure`. This is covered by a unit test on a constructed scenario, not yet by real-log evidence.
...
All numbers here are regenerated by one command (`python training/paper_eval.py`) from committed data. They are listed with their commit in [`docs/EVIDENCE_LEDGER.md`](docs/EVIDENCE_LEDGER.md).

**Benchmark v1:**
- **Data:** 41 real forum logs from 37 incidents, 10 root-cause classes.
- **Grouping:** every log from one incident (source thread) stays on the same side of each split.
- **Labels:** provisional. Most are not yet tied to a written diagnosis ([CORRECTIONS.md, C11](CORRECTIONS.md#c11)).

| Evaluation (log-level macro-F1) | Result | Target | Status |
|---|---|---|---|
| Random guess by label frequency | 0.096 (95th percentile 0.175) | — | chance baseline |
| RandomForest, grouped by incident | **0.092 ± 0.007** | ≥ 0.70 | ❌ at chance level |
| ExtraTrees, grouped by incident | **0.078 ± 0.007** | ≥ 0.70 | ❌ at chance level |
| Rule engine alone | **0.159** (95% CI 0.08–0.28) | ≥ 0.70 | ❌ within chance range |
| Rules + CITA vs rules without CITA | 0.090 vs 0.138 (both 8/41 correct) | CITA helps | ❌ no measurable benefit |
| Same RandomForest, windows split at random (leaky) | 0.890 | — | shows the leakage effect |
| Incidents available | 37 | ≥ 50 | ❌ RELEASE BLOCKED |

**Benchmark v3 (pre-registered, primary):**
- **Data:** 34 real logs from 32 incidents. Every label cites the forum post that diagnosed the failure.
- **Result:** incident-grouped macro-F1 is 0.056–0.069 for RandomForest, ExtraTrees and logistic regression, against a random-guess baseline of 0.076. The same models score 0.78–0.97 with random window splits.
- **Rule engine:** 4/34 correct; with CITA 0/34.
- **Details:** [`docs/PREREGISTRATION.md`](docs/PREREGISTRATION.md) and [`docs/EVIDENCE_LEDGER.md`](docs/EVIDENCE_LEDGER.md). A paper draft is in [`paper/`](paper/).

**Benchmark v2 (labels verified against forum threads):**
- Only 24 of the 55 previously labelled logs have a cause stated in their thread, and only 11 of those 24 matched the old label (Cohen's kappa 0.39).
- On the 22 usable logs, incident-grouped macro-F1 is 0.05 for the tree models and 0.05 for the rule engine, against a random-guess baseline of 0.10.
- The rule engine tends to name a failure's downstream symptom (power drop, motor imbalance) rather than the failed motor itself.
- The v2 labels were assigned by an LLM with cited posts and are not yet human-reviewed. See [`docs/EVIDENCE_LEDGER.md`](docs/EVIDENCE_LEDGER.md).

The leaky row matters. Splitting windows at random lets one flight appear in both training and test, and the same model then looks ten times better. That is how earlier inflated figures arose ([CORRECTIONS.md](CORRECTIONS.md)). The previously reported 0.500 / 0.153 cannot be regenerated and is superseded.

| Engineering check | Result | Target | Status |
|---|---|---|---|
...
<sub>Historical figure: it comes from an earlier evaluation whose inputs can no longer be regenerated, and it is not current evidence. Current calibration figures are in <code>docs/EVIDENCE_LEDGER.md</code>.</sub>
</div>
...
| **Evaluation** | Real-log benchmark v1: 41 logs, 37 incidents, incident-grouped 5-fold CV × 5 seeds |
| **Result** | Incident-grouped macro-F1 0.09 (chance 0.10); ECE 0.08; release gates fail |
...
1. On the reproducible real-log benchmark, incident-grouped macro-F1 is at chance level (0.08–0.09 for the tree models, 0.16 for the rule engine; gate 0.700). See [`docs/EVIDENCE_LEDGER.md`](docs/EVIDENCE_LEDGER.md).
2. Only 37 real incidents are available (gate: 50), and most labels are not yet tied to a written diagnosis.
