...
[![Real-log macro F1 (incident-grouped): 0.09, chance 0.10](https://img.shields.io/badge/real--log%20macro%20F1%20(incident--grouped)-0.09%20%7C%20chance%200.10-red)](docs/EVIDENCE_LEDGER.md)
...
> **Measured status:** CITA shows **no measurable benefit**. On benchmark v1 it changed 20 of 41 top predictions, and 8 were correct either way. On the thread-verified v2 labels, rules with CITA get 0 of 22 right, against 1 without ([evidence](docs/EVIDENCE_LEDGER.md)). The labels are provisional and have no onset annotations. A proper test needs annotated onset times, which is planned in [`docs/PUBLICATION_PLAN.md`](docs/PUBLICATION_PLAN.md). Treat the description below as the design intent.
